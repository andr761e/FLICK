"""Unreal commandlet: import the visual-only BOB Pocket Foundry venue."""
...
parent_path = destination + "/M_BobStadiumSurface"
parent = u.load_asset(parent_path)
if not parent:
    parent = assets.create_asset("M_BobStadiumSurface", destination, u.Material, u.MaterialFactoryNew())
    ...
    base = parameter(u.MaterialExpressionVectorParameter, "BaseColor",
                     u.LinearColor(.1, .14, .16, 1), u.MaterialProperty.MP_BASE_COLOR)
    parameter(u.MaterialExpressionScalarParameter, "Metallic", .4, u.MaterialProperty.MP_METALLIC)
    parameter(u.MaterialExpressionScalarParameter, "Roughness", .35, u.MaterialProperty.MP_ROUGHNESS)
...
for name in manifest["exports"]:
    ...
    if not isinstance(mesh, u.StaticMesh):
        raise RuntimeError("BOB stadium mesh import failed: " + name)
    ...
    for index, slot in enumerate(mesh.get_editor_property("static_materials")):
        ...
        if len(matches) != 1:
            raise RuntimeError(f"Unknown BOB stadium material slot on {name}: {slot_name}")
