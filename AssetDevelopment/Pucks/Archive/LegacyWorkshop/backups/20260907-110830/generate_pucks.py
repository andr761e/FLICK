...
for index,name in enumerate(names):
    ...
    cyl(name+'_Body',r*.985,h*(.62 if name=='Bouncer' else .82),-h*(.14 if name=='Bouncer' else .04),dark)
    ...
    cyl('Center insert',r*.625,h*.065,h*(.22 if name=='Bouncer' else .412),inset)
    ...
    for j in range(segments):
        ...
        if name in ['Heavy','Grippy']:
            arc('Reinforced grip lug',r*.93,r,0,h*.82,a-.055,a+.055,rubber if name=='Grippy' else silver,8)
    z=h*.465; s=r*.31; w=r*.038
    ...
    if name in ['Standard','Compact','Striker']:
        ...
        if name!='Standard': cyl('Class dot',s*.16,h*.014,z,cyan)
        if name=='Striker':
            for j in range(4):
                a=j*math.pi/2
                line('Sight tick',[(s*v*math.cos(a),s*v*math.sin(a)) for v in [.65,1.3]],z,w,cyan)
    elif name=='Toppler':
        for y in [-.38,.30]: stroke([(-.85,y),(0,y+.55),(.85,y)])
    elif name=='Slider':
        for x in [-.45,.30]: stroke([(x-.35,-.65),(x+.35,0),(x-.35,.65)])
    elif name=='Blocker':
        stroke([(-.78,.6),(0,.92),(.78,.6),(.68,-.45),(0,-.95),(-.68,-.45),(-.78,.6)])
        stroke([(-.48,.38),(0,.59),(.48,.38),(.42,-.25),(0,-.59),(-.42,-.25),(-.48,.38)])
    elif name=='Grippy': stroke([(-1,-.65),(-.25,.5),(0,.25),(.25,.85),(1,-.65),(0,-.02),(-1,-.65)])
    elif name=='Heavy':
        stroke([(-.8,0),(.8,0)])
        for x in [-.8,.8]: stroke([(x,-.65),(x,.65)])
        for x in [-1.1,1.1]: stroke([(x,-.35),(x,.35)])
    else:
        cyl('Bouncer dome seat',r*.34,h*.06,h*.23,rubber)
        bpy.ops.mesh.primitive_uv_sphere_add(segments=48,ring_count=24,radius=1,location=(0,0,h*.24))
        o=bpy.context.object; o.scale=(r*.23,r*.23,h*.26); finish(o,'Bouncer luminous dome',cyan)
