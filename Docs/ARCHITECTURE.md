...
`AFlickCameraPawn` owns the slow main-menu arena orbit, gameplay viewpoints, top-down and free camera modes, post processing, and cosmetic impact shake. Mouse deprojection and gamepad screen axes read the live camera transform, so changing sides does not require alternate shot math.
...
`FlickPieceArchetypeRules` defines the physical and visual multipliers for all nine Knockout pucks: Standard, Heavy, Striker, Grippy, Slider, Blocker, Compact, Bouncer, and Toppler. Players save four named classes, each containing four distinct archetypes. GameMode composes the selected archetype with the active mode's base values before spawning each Chaos body.

`FlickCosmeticCatalog` defines the locker categories and their currently available items. The first three categories retain the existing banner, tag, and border save keys; each puck archetype has its own appearance category and save key. Only the Original appearance exists for each puck today. The main-menu display-puck selection is a separate setting from the equipped appearance.
...
`AFlickGameMode` remains one authority class, but its implementation is grouped by responsibility so match behavior is not buried in one translation unit:

- `FlickGameMode.cpp`: lifecycle, login/logout, and the main tick.
- `FlickGameModeShot.cpp`: timers, shot validation, launches, and impacts.
- `FlickGameModeFrontend.cpp`: mode/loadout/class selection and training setup.
- `FlickGameModeOnline.cpp`: sessions, parties, lobbies, matchmaking, and ranked flow.
- `FlickGameModeRuntime.cpp`: settings, spawning, camera routing, and turn scheduling.
- `FlickGameModeResolution.cpp`: replay, elimination, settlement, scoring, and round completion.

`SFlickGameLayer` follows the same convention: the root file owns construction and input routing, while `Frontend`, `Play`, `Lineup`, `Match`, and `MatchMenus` files own their respective screens. Shared internal Slate primitives live in `FlickGameLayerPrivate.h`; they are implementation details and should not be included outside the game-layer source files.

Run `Tools/Audit-FlickAssets.py` through Unreal's Python commandlet when removing content. The report in `Saved/FlickAssetAudit.json` combines Asset Registry referencers with literal and formatted runtime asset paths, avoiding false orphan reports for dynamically loaded puck variants.

## Frontend Flow

The runtime starts in `MainMenu` with a slow arena orbit behind a translucent, diagonal left-side navigation panel. Play enters the Casual, Competitive, Training, and Private Match selection flow, with 1v1, 2v2, 3v3, and BOB playlists where applicable. Knockout uses a saved four-puck class; BOB forces Standard pucks without modifying saved Knockout choices. Selecting an archetype already present in a class swaps it into the chosen slot rather than creating a duplicate. Starting a match blends to the competitive camera and rebuilds the selected arena. Each Knockout round begins with simultaneous kickoff planning, then alternates normal turns; there is no fixed round ceiling, and the first team to win three rounds wins the match. Drawn rounds award no win. BOB opens directly in `Aiming` with both persistent strikers on their owner baselines and ends after one player clears their color. Escape enters `Paused`; Settings remembers whether Back should return to pause or the main menu. Returning home rebuilds the menu preview and clears any in-progress aim.
...
Private matches use an Unreal listen server; coordinator-allocated public matches use a separate dedicated Unreal authority. Players submit piece identity, aim direction, and normalized power through server RPCs; GameMode validates ownership and turn state before launching the authoritative replicated puck. Match phase, teams, readiness, scores, and lobby mode are replicated. Steam supplies identity, invitations and transport but does not decide teams, ratings, admission, or gameplay outcomes. Direct-IP sessions exercise the same replicated gameplay code through the fallback IP driver.

`UFlickMatchmakingCoordinatorSubsystem` is the production matchmaking boundary. It submits retry-safe complete-party tickets with a network build ID, reports expanding queue state, retains reconnect reservations, and lets dedicated authorities verify admission and report lifecycle transitions. The coordinator owns playlist ratings and team/slot assignment, keeps parties intact, and withholds allocations until their server heartbeats. `UFlickSessionSubsystem` remains responsible for Steam friends, invitations, private parties, browsing, and the development session-matching fallback. Ranked identity and settlement remain server-only through `UFlickRankedBackendSubsystem`; neither Steam lobby metadata, client rating snapshots, nor client travel options are authoritative.
