...
The runtime starts in `MainMenu` with a non-interactive cinematic preview behind a translucent, diagonal left-side navigation panel. Play enters the Casual, Competitive, Training, and Private Match selection flow, with 1v1, 2v2, 3v3, and BOB playlists where applicable. Knockout can start with saved lineups or route into `Loadout`; BOB hides that route and forces Standard pucks without modifying saved Knockout choices. Each Knockout player brings four distinct puck archetypes, and selecting an archetype already present swaps it into the chosen slot rather than creating a duplicate. Starting a match blends to the competitive camera and rebuilds the selected arena. Every new aiming turn begins at that player's home-side view; before selecting a puck, the player may rotate through all four board sides. Selection is suspended during the short camera blend, and an active drag prevents rotation. Knockout rounds open in `KickoffPlanning` and continue without a fixed round ceiling until one side wins three rounds; drawn rounds award no win. BOB opens directly in `Aiming` with both persistent strikers on their owner baselines and ends after one player clears their color. Escape enters `Paused`; Settings remembers whether Back should return to pause or the main menu. Returning home rebuilds a fresh preview and clears any in-progress aim.
