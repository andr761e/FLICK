...
`AFlickAudioDirector` synthesizes short PCM cues at runtime and plays them as procedural sound waves. Physics events provide strength, mass, speed, class, and location inputs; the audio layer never modifies gameplay state. Sliding uses velocity-driven granular scrape events rather than a permanently allocated loop.
